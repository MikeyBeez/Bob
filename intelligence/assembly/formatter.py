...
class ContextFormatter:
    ...
    def format_context(self, context_items: List[Any], 
        ...
        elif format_type == FormatType.CONVERSATIONAL:
            formatted = self._format_as_conversational(sections)
    ...
    def _format_as_conversational(self, sections: List[FormattedSection]) -> str:
        """Format as natural conversational context."""
        if not sections:
            return ""
        ...
        output.append("Here's what I found that's relevant to your question:")
        output.append("")
        
        # Present sections naturally
        for i, section in enumerate(sections):
            if i > 0:
                output.append("Additionally:")
                output.append("")
            
            # Remove overly formal structure, make it flow naturally
            content = section.content.strip()
            
            # Add some conversational context if needed
            if section.source_info.get('source_id'):
                source_hint = self._get_friendly_source_name(section.source_info.get('source_id', ''))
                if source_hint:
                    output.append(f"From {source_hint}:")
    ...
    def _get_friendly_source_name(self, source_id: str) -> str:
        """Convert technical source IDs to friendly names."""
        if 'database' in source_id.lower():
            return "your stored information"
        elif 'filesystem' in source_id.lower() or 'file' in source_id.lower():
            return "your files"
        elif 'memory' in source_id.lower():
            return "recent conversations"
        elif 'api' in source_id.lower():
